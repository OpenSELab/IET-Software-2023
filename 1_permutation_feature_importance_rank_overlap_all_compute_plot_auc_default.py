# -*- coding: utf-8 -*-
"""1-permutation_feature_importance_rank_overlap_all_compute_plot_auc_default

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/16sd9M2rAnPlbY_pvpgSgB1XUN5NpqPAq
"""
...
data_o_Promise = pd.DataFrame(pd.read_csv(rootpath + datasets_original + "Promise-permutation-test-auc-rank.csv"))

data_o_ReLink = pd.DataFrame(pd.read_csv(rootpath + datasets_original + "ReLink-permutation-test-auc-rank.csv"))

data_o_AEEEM = pd.DataFrame(pd.read_csv(rootpath + datasets_original + "AEEEM-permutation-test-auc-rank.csv"))

data_log_Promise = pd.DataFrame(pd.read_csv(rootpath + datasets_log + "Promise-permutation-test-auc-rank.csv"))

data_log_ReLink = pd.DataFrame(pd.read_csv(rootpath + datasets_log + "ReLink-permutation-test-auc-rank.csv"))

data_log_AEEEM = pd.DataFrame(pd.read_csv(rootpath + datasets_log + "AEEEM-permutation-test-auc-rank.csv"))


data_min_Promise = pd.DataFrame(pd.read_csv(rootpath + datasets_minmax + "Promise-permutation-test-auc-rank.csv"))

data_min_ReLink = pd.DataFrame(pd.read_csv(rootpath + datasets_minmax + "ReLink-permutation-test-auc-rank.csv"))

data_min_AEEEM = pd.DataFrame(pd.read_csv(rootpath + datasets_minmax + "AEEEM-permutation-test-auc-rank.csv"))

data_sta_Promise = pd.DataFrame(pd.read_csv(rootpath + datasets_standardize + "Promise-permutation-test-auc-rank.csv"))

data_sta_ReLink = pd.DataFrame(pd.read_csv(rootpath + datasets_standardize + "ReLink-permutation-test-auc-rank.csv"))

data_sta_AEEEM = pd.DataFrame(pd.read_csv(rootpath + datasets_standardize + "AEEEM-permutation-test-auc-rank.csv"))
...
for k in range(len(O)):
  ...
  for i in range(O[k].shape[0]):
    ...
    for j in range(O[k].shape[1]):
      ...
      if(O[k].loc[i].values[j] == 1):  # =1 overlap-1 =3 overlap-3
        old_1_attrlist.append(list(O[k])[j])

      if(L[k].loc[i].values[j] == 1):  # =1 overlap-1 =3 overlap-3
        new_1_attrlist.append(list(L[k])[j])



      if(O[k].loc[i].values[j] == 1 or O[k].loc[i].values[j] == 2 or O[k].loc[i].values[j] == 3):  # =1 overlap-1 =3 overlap-3
        old_3_attrlist.append(list(O[k])[j])

      if(L[k].loc[i].values[j] == 1 or L[k].loc[i].values[j] == 2 or L[k].loc[i].values[j] == 3):  # =1 overlap-1 =3 overlap-3
        new_3_attrlist.append(list(L[k])[j])


      if(O[k].loc[i].values[j] == 1 or O[k].loc[i].values[j] == 2 or O[k].loc[i].values[j] == 3 or O[k].loc[i].values[j] == 4 or O[k].loc[i].values[j] == 5):  # =1 overlap-1 =3 overlap-3
        old_5_attrlist.append(list(O[k])[j])

      if(L[k].loc[i].values[j] == 1 or L[k].loc[i].values[j] == 2 or L[k].loc[i].values[j] == 3 or L[k].loc[i].values[j] == 4 or L[k].loc[i].values[j] == 5):  # =1 overlap-1 =3 overlap-3
        new_5_attrlist.append(list(L[k])[j])


    print("1交集")
    print(list(set(old_1_attrlist).intersection(set(new_1_attrlist))))
    print("1并集")
    print(list(set(old_1_attrlist).union(set(new_1_attrlist))))

    print("3交集")
    print(list(set(old_3_attrlist).intersection(set(new_3_attrlist))))
    print("3并集")
    print(list(set(old_3_attrlist).union(set(new_3_attrlist))))

    print("5交集")
    print(list(set(old_5_attrlist).intersection(set(new_5_attrlist))))
    print("5并集")
    print(list(set(old_5_attrlist).union(set(new_5_attrlist))))

    #print(old_attrlist) #输出证明有的的确是多个属性同时为1 rank diff 没算错
    #print(new_attrlist)

    # 交集/并集
    overlap_1 = len(list(set(old_1_attrlist).intersection(set(new_1_attrlist)))) / len(list(set(old_1_attrlist).union(set(new_1_attrlist))))
    overlap_3 = len(list(set(old_3_attrlist).intersection(set(new_3_attrlist)))) / len(list(set(old_3_attrlist).union(set(new_3_attrlist))))
    overlap_5 = len(list(set(old_5_attrlist).intersection(set(new_5_attrlist)))) / len(list(set(old_5_attrlist).union(set(new_5_attrlist))))

    overlap_1_list.append(overlap_1)
    overlap_3_list.append(overlap_3)
    overlap_5_list.append(overlap_5)


print("overlap_1_list")
print(overlap_1_list)
print(len(overlap_1_list))  # 为什么不是1800？因为有某个行存在两个1

print("overlap_3_list")
print(overlap_3_list)
print(len(overlap_3_list))  # 为什么不是1800？因为有某个行存在两个1

print("overlap_5_list")
print(overlap_5_list)
print(len(overlap_5_list))  # 为什么不是1800？因为有某个行存在两个1
...
with sns.axes_style('darkgrid'):
  fig, axes = plt.subplots(3, 3, figsize=(12, 6), sharey=True, sharex=True, dpi=350) # sharey=True , dpi=350
  fig.subplots_adjust(wspace=0.010,hspace=0.050)

  #fig,axes=plt.subplots(1,3, figsize=(15, 4))

  #axes[0].set_title(  "Year Period:2005-2015",loc='center', fontsize=20  )
  axes[0][0].axvline(0, color='black', linestyle=':')
  axes[0][0].axvline(0.25, color='black', linestyle=':')
  axes[0][0].axvline(0.5, color='black', linestyle=':')
  axes[0][0].axvline(0.75, color='black', linestyle=':')
  ...
  axes[0][0].set_xticks([0.0,0.5,1.0])
  axes[0][0].set_yticks([0.0,0.5,1.0,1.5,2.0])
  sns.distplot(overlap_L_O_1_list, hist=False, kde=True, color='darkblue', kde_kws=dict(linewidth=2), ax=axes[0][0])
  #axes[0].set_ylabel( "Probability density", fontsize=20, labelpad=10 )
  axes[0][0].set_ylabel( "Log-O")
  ...
  axes[0][1].set_xticks([0.0,0.25,0.5,0.75,1.0])
  axes[0][1].set_yticks([0.0,0.5,1.0,1.5,2.0])
  sns.distplot(overlap_L_O_3_list, hist=False, kde=True, color='darkblue', kde_kws=dict(linewidth=2), ax=axes[0][1])
  #axes[0].set_ylabel( "Probability density", fontsize=20, labelpad=10 )
  #axes[0].set_xlabel( "Temperature:K",fontsize=20 ,labelpad=10)
  ...
  axes[0][2].set_xticks([0.0,0.25,0.5,0.75,1.0])
  axes[0][2].set_yticks([0.0,0.5,1.0,1.5,2.0])
  sns.distplot(overlap_L_O_5_list, hist=False, kde=True, color='darkblue', kde_kws=dict(linewidth=2), ax=axes[0][2])
  #axes[0].set_ylabel( "Probability density", fontsize=20, labelpad=10 )
  #axes[0].set_xlabel( "Temperature:K",fontsize=20 ,labelpad=10)


  #axes[0].set_title(  "Year Period:2005-2015",loc='center', fontsize=20  )
  axes[1][0].set_ylabel( "Min-O")
  axes[1][0].axvline(0, color='black', linestyle=':')
  axes[1][0].axvline(0.25, color='black', linestyle=':')
  axes[1][0].axvline(0.5, color='black', linestyle=':')
  axes[1][0].axvline(0.75, color='black', linestyle=':')
  ...
  axes[1][0].set_xticks([0.0,0.5,1.0])
  axes[1][0].set_yticks([0.0,0.5,1.0,1.5,2.0])
  sns.distplot(overlap_M_O_1_list, hist=False, kde=True, color='darkorange', kde_kws=dict(linewidth=2), ax=axes[1][0])
  ...
  axes[1][1].set_xticks([0.0,0.25,0.5,0.75,1.0])
  axes[1][1].set_yticks([0.0,0.5,1.0,1.5,2.0])
  sns.distplot(overlap_M_O_3_list, hist=False, kde=True, color='darkorange', kde_kws=dict(linewidth=2), ax=axes[1][1])
  ...
  axes[1][2].set_xticks([0.0,0.25,0.5,0.75,1.0])
  axes[1][2].set_yticks([0.0,0.5,1.0,1.5,2.0])
  sns.distplot(overlap_M_O_5_list, hist=False, kde=True, color='darkorange', kde_kws=dict(linewidth=2), ax=axes[1][2])
  #axes[0].set_ylabel( "Probability density", fontsize=20, labelpad=10 )
  #axes[0].set_xlabel( "Temperature:K",fontsize=20 ,labelpad=10)


  #axes[0].set_title(  "Year Period:2005-2015",loc='center', fontsize=20  )
  axes[2][0].set_ylabel( "Sta-O")
  axes[2][0].axvline(0, color='black', linestyle=':')
  axes[2][0].axvline(0.25, color='black', linestyle=':')
  axes[2][0].axvline(0.5, color='black', linestyle=':')
  axes[2][0].axvline(0.75, color='black', linestyle=':')
  ...
  axes[2][0].set_xticks([0.0,0.5,1.0])
  axes[2][0].set_yticks([0.0,0.5,1.0,1.5,2.0])
  sns.distplot(overlap_S_O_1_list, hist=False, kde=True, color='darkgreen', kde_kws=dict(linewidth=2), ax=axes[2][0])
  #axes[2].set_ylabel( "Probability density", fontsize=20, labelpad=10 )
  #axes[2].set_xlabel( "Temperature:K",fontsize=20 ,labelpad=10)
  ...
  axes[2][1].set_xticks([0.0,0.25,0.5,0.75,1.0])
  axes[2][1].set_yticks([0.0,0.5,1.0,1.5,2.0])
  sns.distplot(overlap_S_O_3_list, hist=False, kde=True, color='darkgreen', kde_kws=dict(linewidth=2), ax=axes[2][1])
  #axes[4].set_ylabel( "Probability density", fontsize=20, labelpad=10 )
  #axes[4].set_xlabel( "Temperature:K",fontsize=20 ,labelpad=10)
  ...
  axes[2][2].set_xticks([0.0,0.25,0.5,0.75,1.0])
  axes[2][2].set_yticks([0.0,0.5,1.0,1.5,2.0])
  sns.distplot(overlap_S_O_5_list, hist=False, kde=True, color='darkgreen', kde_kws=dict(linewidth=2), ax=axes[2][2])
  #axes[0].set_ylabel( "Probability density", fontsize=20, labelpad=10 )
  #axes[0].set_xlabel( "Temperature:K",fontsize=20 ,labelpad=10)

results_overlap = "results/overlap/"

count_L_O_1_pd = pd.DataFrame(pd.read_excel( rootpath + results_overlap + "count_L_O_1_pd.xlsx" ))
count_L_O_3_pd = pd.DataFrame(pd.read_excel( rootpath + results_overlap + "count_L_O_3_pd.xlsx" ))
count_L_O_5_pd = pd.DataFrame(pd.read_excel( rootpath + results_overlap + "count_L_O_5_pd.xlsx" ))
count_M_O_1_pd = pd.DataFrame(pd.read_excel( rootpath + results_overlap + "count_M_O_1_pd.xlsx" ))
count_M_O_3_pd = pd.DataFrame(pd.read_excel( rootpath + results_overlap + "count_M_O_3_pd.xlsx" ))
count_M_O_5_pd = pd.DataFrame(pd.read_excel( rootpath + results_overlap + "count_M_O_5_pd.xlsx" ))
count_S_O_1_pd = pd.DataFrame(pd.read_excel( rootpath + results_overlap + "count_S_O_1_pd.xlsx" ))
count_S_O_3_pd = pd.DataFrame(pd.read_excel( rootpath + results_overlap + "count_S_O_3_pd.xlsx" ))
count_S_O_5_pd = pd.DataFrame(pd.read_excel( rootpath + results_overlap + "count_S_O_5_pd.xlsx" ))